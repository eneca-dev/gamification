...
Управление курсом кристаллов: на странице `/admin/products` сверху таблицы — карточка `CrystalRateHistory` в режиме редактирования. Три колонки в ряд (равной высоты): слева текущий курс + история изменений, в центре установка нового курса (инпут + тоггл «Предпросмотр», локально пересчитывающий цены в таблице товаров без записи в БД, + кнопка «Применить»), справа статистика (`CrystalRateStats`): сколько дней держится курс + счётчики товаров по статусам. При применении — INSERT в `crystal_rates`, инвалидация тегов `crystal-rate` и `shop-products`. На `/admin/economy` тот же компонент рендерится без `editing` и `extra` — одна карточка на всю ширину, только просмотр.
...
- `economy/EconomyDashboard` — серверный контейнер дашборда экономики. Принимает `period`/`customFrom`/`customTo`/`topLevel`/`overview`/`categories`/`tops` и рендерит `EconomyFilters` → `KpiSummary` → `SpendingBreakdown` → `CategoryBreakdownChart` → grid из 6 `TopList`
- `economy/EconomyFilters` — клиентский компонент фильтров: пресеты периода (7д/30д/90д/Год/Всё время/Период), `DateRangePicker` для кастомного диапазона, переключатель уровня топов (Сотрудники/Команды/Отделы). Все изменения через `router.replace` в URL searchParams (`useTransition` для индикации загрузки). Чипсы вынесены в локальный helper `ChipButton`
- `economy/KpiSummary` — серверный блок «Сводка»: 4 KPI-карточки (заработано / фактически заработано / отозвано фактически / подарено компанией) + строка «Не хватило баланса на полный штраф в N из M отзывов (X%)». Принимает `rate`. BYN-сумма — основное число (24px bold), под ней `CoinStatic` size="sm" в muted-цвете
- `economy/SpendingBreakdown` — серверный блок «Куда уходят деньги»: 5 карточек по каналам (магазин, лотерея, вторая жизнь, платные благодарности, квотные благодарности), каждая с BYN-суммой как основным числом (20px bold), `CoinStatic` size="sm" вторичным и количеством участников. Принимает `rate`. Для plurals — локальный helper `pluralize`
- `economy/TopList` — клиентский переиспользуемый топ: заголовок с иконкой, превью первых 10 позиций (BYN-сумма основная, `CoinStatic` size="xs" вторичная), кнопка «Показать всех» открывает модалку (через портал) с полным списком. Принимает `rate`. Опциональный `secondaryLabel` для счётчика транзакций (покупок / отправок / билетов)
- `economy/CategoryBreakdownChart` — клиентский блок «Категории магазина»: Recharts donut + легенда-список. BYN — основное число в строке категории, товара и tooltip; `CoinStatic` size="xs" — вторичная подпись. Принимает `rate`
...
- `ProductsClient` — управление товарами и категориями: inline-редактирование (cost_byn, coefficient, category, stock), поиск, фильтрация по категориям (динамический overflow пиллов), удаление товаров, создание/редактирование через модалку. Принимает `currentRate` и `crystalRates`, держит локальный `previewRate` state. Цена в кристаллах в таблице — вычисляется через `effectiveRate = previewRate ?? currentRate`. Рендерит `CrystalRateHistory` сверху в режиме редактирования (передаёт `editing` и `extra` с `CrystalRateStats`; счётчики статусов товаров считает через `useMemo` из products)
- `CrystalRateHistory` (`components/economy/`) — карточка курса кристаллов: текущий курс, история изменений. Опциональный проп `editing` (previewRate, onPreviewRateChange, onError, onApplied) включает вторую колонку — управление курсом: инпут «Новый курс», тоггл «Предпросмотр», кнопка «Применить» (вызывает `setCrystalRate` через useTransition, disabled при равенстве draft и currentRate). Опциональный проп `extra` (ReactNode) — третья колонка, занимает оставшееся место. При наличии `editing`/`extra` колонки выстраиваются в ряд (lg), блок курса сжимается до фикс. ширины; иначе одна карточка на всю ширину. Без `editing` — read-only (используется на `/admin/economy`). currentRate берётся из `rates[0].rate`
- `CrystalRateStats` — статистика для блока курса (передаётся в `CrystalRateHistory.extra` на `/admin/products`): сколько дней держится текущий курс (по `ratedAt`) + сетка показателей (всего товаров, категорий, со скидкой, активно/скоро/неактивно, нет в наличии, средняя и макс. цена). Чистый презентационный компонент; `stats` считается в `ProductsClient` через `useMemo` из products/categories. Цены берутся по `effectiveRate`, поэтому реагируют на предпросмотр курса
