...
Поток данных:
1. Edge functions синкают сырые данные из внешних систем в source-таблицы (`plugin_launches`, `gratitudes`, `planning_freshness`, `ws_daily_status`).
...
- **Elasticsearch / Kibana** — источник `plugin_launches`
- **Airtable** — источник `gratitudes`
- **eneca.work Supabase** — источник `planning_freshness`, `planning_freshness_daily`
...
Благодарности из Airtable. Синкается edge function `sync-gratitudes` (только текущий месяц).
...
#### `planning_freshness_daily`

Снапшоты `planning_freshness` за каждый день. Заполняется при вызове `sync-planning-freshness?snapshot=true`.
...
| `source_type` | text NULL | `plugin_launches` / `gratitudes` / `planning_freshness` / `streaks` / `admin` |
...
|---|---|---|---|
| `trg_award_revit_points` | `plugin_launches` | `fn_award_revit_points()` | +5 за зелёный день, обновляет стрик, бонус при 7/30 |
| `trg_award_gratitude_points` | `gratitudes` | `fn_award_gratitude_points()` | +20 получателю, +10 отправителю (лимит 3/нед) |
| `trg_award_planning_points` | `planning_freshness` | `fn_award_planning_points()` | +30 бонус (≤3 дня) или -30 штраф (>7 дней) |
...
| `plugin_launches` | authenticated | service_role | service_role | запрещено |
| `gratitudes` | authenticated | service_role | service_role | запрещено |
| `planning_freshness` | authenticated | service_role | service_role | запрещено |
| `planning_freshness_daily` | authenticated | service_role | service_role | запрещено |
...
| `gamification_settings` | authenticated | service_role | service_role | запрещено |
