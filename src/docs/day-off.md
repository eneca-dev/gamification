...
Сотрудник заполняет форму: дата (только будущие), необязательный комментарий, обязательный скриншот переписки с руководителем. Скриншот загружается в Storage-бакет `day-off-screenshots`.
...
- Дата заявки — только будущая (валидация в Zod + input min)
