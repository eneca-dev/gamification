...
**«Презумпция невиновности»:** если задача хотя бы раз была в статусе «В работе» в течение дня (проверяется через `ws_task_status_changes` из get_events API), штраф не применяется. `sync-task-events` отслеживает события WS `update`, `close` и `reopen` — нативное закрытие/переоткрытие задачи в WS тоже несёт `old`/`new.tags` и обрабатывается той же логикой. Закрытие («Готово») в тот же день само по себе штраф **не** отменяет — часы могут вноситься в уже закрытую задачу без реального возврата в работу.

NULL-статус (тег не установлен) = штраф.

**Освобождение по типу задачи:** если в имени задачи (case-insensitive) встречается `курирование`, `совещание` или `совещания` — штраф не начисляется. Метки Worksection (`{Курирование}`, `{Совещание}`) автоматически попадают в имя из WS API, поэтому покрываются той же проверкой.
