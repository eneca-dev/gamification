# План: Соревнование отделов — метрика по баллам + начисление бонуса

## Контекст

Соревнование отделов по автоматизации (Revit). Сейчас метрика — "% сотрудников использующих плагины". Нужно: сумма баллов за ревит по отделу. Плюс автоматическое начисление бонуса 1 числа каждого месяца.

## Что делаем

### Шаг 1. VIEW `view_department_revit_contest` в БД

Считает сумму ревит-баллов по отделам за текущий месяц. Один SELECT вместо двух запросов + JS-агрегации.
