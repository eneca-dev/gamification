...
- `compute-gamification.ts` — шаг 6 (создание транзакций): убрать условие, которое пропускает события с 0 💎, или явно проверять стоимость из `gamification_event_types`.
...
1. **Ретроактивное начисление green_day:** нет. +3 💎 начисляются только с момента применения миграции. Прошлые зелёные дни остаются без транзакций.
