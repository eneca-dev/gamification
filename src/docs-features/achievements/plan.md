...
|---|---|---|---|---|
| `ach_revit_leader` | Лидер автоматизации | individual | Топ-1 по **баллам** за Revit за квартал | 2000 коинов |
| `ach_revit_team` | Технологичная команда | team | Топ-1 команда по среднему кол-ву баллов на сотрудника | +500 каждому в команде |
| `ach_revit_department` | Цифровой авангард | department | Топ-1 отдел по % вовлечённости в автоматизации | Пицца / баллы на отдел |
...
| ID | Название | Scope | Условие | Бонус |
|---|---|---|---|---|
| `ach_culture_support` | Поддержка коллег | individual | Топ-1 по количеству полученных благодарностей за квартал | 1000 коинов |
| `ach_culture_cross` | Межфункциональное взаимодействие | individual | Благодарности от 3+ разных отделов за квартал | 600 коинов |
...
| ID | Название | Scope | Условие | Бонус |
|---|---|---|---|---|
| `ach_ws_discipline` | Эталонная дисциплина | individual | 0 красных дней за 12 месяцев | 1000 коинов |
...
| `ach_culture_mentor` | Наставничество | individual | 5+ благодарностей в категории "Обучение/Менторство" | 1200 коинов |
...
CREATE TABLE ach_evaluations (
  ...
  winner_value         numeric,                 -- метрика победителя (367 баллов, 95% и т.д.)
...
CREATE TABLE ach_awards (
  ...
  event_id         uuid REFERENCES gamification_event_logs(id),  -- связь с начислением коинов
...
Баллы за достижения попадают в существующую систему логов и транзакций.
...
ach_revit_department   | ???  | Достижение: Цифровой авангард (пицца или баллы — уточнить)
...
```
1. Для каждой команды: среднее кол-во revit-баллов на сотрудника
...
**Q1: "Баллы" vs "запуски" для Revit**
Сейчас `revit_using_plugins` = 5 коинов за **день** использования (не за каждый запуск).
Человек с 367 запусками и человек с 10 запусками за день получают одинаковые 5 коинов.
"Лидер автоматизации" по баллам = лидер по количеству **дней**, а не запусков.
...
**Q3: Пицца для отдела — баллы или вне системы?**
Для "Цифрового авангарда" — "пицца ИЛИ баллы на командный счёт".
→ **Сколько баллов и кому? Каждому сотруднику? Или это вне приложения?**
