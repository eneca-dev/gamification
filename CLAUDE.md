...
> **Пайплайн разработки:** `src/docs/dev-pipeline.md` — читать перед началом любой задачи.
...
**Иконка валюты (CoinIcon):**
- Для отображения валюты (монет) **всегда** использовать `<CoinIcon />` из `@/components/CoinIcon`
- Для отображения суммы с иконкой — `<CoinBalance />`, `<CoinStatic />` или `<CoinBalanceLive />` из `@/components/CoinBalance`
- Запрещено использовать эмодзи (💰, 🪙, 🟡), текст ("монет", "coins") или другие иконки вместо `CoinIcon`
- Размер иконки задаётся через prop `size` (число в пикселях), не через CSS
...
| Клиентский кэш / мутации | `@/modules/cache` | Интерактивные списки, оптимистичные обновления, realtime |

**Cache Module** (`@/modules/cache`) — обязательно использовать когда:
- Компонент интерактивный и данные нужны на клиенте
- Нужны оптимистичные обновления
- Нужна realtime-синхронизация с БД

Прямые импорты из `@tanstack/react-query` в компонентах и модулях запрещены — только через фабрики `createCacheQuery`, `createCacheMutation` и т.д. из `@/modules/cache`.
...
- **cache** — централизованный кэш на TanStack Query: ActionResult, query/mutation factories, realtime sync. Документация: `src/modules/cache/README.md`
...
Вся разработка ведётся по пайплайну, описанному в `src/docs/dev-pipeline.md`.

- **Quick Pipeline** — баги, отдельные компоненты, хуки, UI-изменения (1-5 файлов)
- **Full Pipeline** — новые модули, крупные фичи, изменения в схеме БД (> 5 файлов)

Перед началом задачи — определить масштаб и выбрать соответствующий пайплайн.
...
В корне проекта есть `graphify-out/` — граф зависимостей всего проекта (src/ + supabase/).

**Если `graphify-out/` существует — использовать граф.** Перед анализом зависимостей, поиском модулей и планированием изменений всегда обращаться к графу, а не угадывать структуру по именам файлов.

### Когда использовать
- **Перед началом любой задачи, затрагивающей > 1 модуля** — выполнить `/graphify query "<вопрос>"` чтобы понять связи
- **При поиске места для новой логики** — `/graphify path "ModuleA" "ModuleB"` покажет как они связаны
- **При неясной зависимости** — `/graphify explain "ComponentName"` покажет всё что к нему относится

### Когда обновлять (`/graphify . --update`)
- После добавления нового модуля или значительного рефакторинга
- После изменений в `src/docs/` или `supabase/migrations/`
- Перед началом крупной фичи (Full Pipeline)
- Не нужно: при правке одного компонента, баг-фиксах, UI-изменениях

### Файлы
- `graphify-out/graph.json` — данные графа (не коммитится)
- `graphify-out/GRAPH_REPORT.md` — отчёт с god nodes и surprising connections
- `graphify-out/graph.html` — интерактивная визуализация (открыть в браузере)
