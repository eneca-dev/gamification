-- Cutoff revoke-транзакций по фактической дате закрытия задачи (не по event_date).
-- Резолвим через JOIN с ws_tasks_l3/l2 по ws_task_id из details
-- (для budget_revoked_*, deadline_revoked_l3 — есть один задача-источник).
-- Для master_planner_*_revoked (нет single task id) и редких случаев когда задача удалена
-- — фоллбек на details.original_details.date_closed, затем event_date.
