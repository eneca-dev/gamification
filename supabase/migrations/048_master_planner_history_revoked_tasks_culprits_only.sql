-- Откатываю COALESCE из миграции 047: revoked_tasks теперь только details.revoked_tasks
-- (задачи, реально сорвавшие серию). Для bulk-amnesty событий (где revoked_tasks отсутствует
-- и есть только original_details.tasks) поле станет NULL — UI не должен показывать 10 «виновников»,
-- т.к. амнистия аннулирует серию целиком, а не из-за конкретной задачи.
