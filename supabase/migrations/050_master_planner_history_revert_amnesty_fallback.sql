-- Откат COALESCE из миграции 049: revoked_tasks снова только details.revoked_tasks.
-- Колонка is_amnesty остаётся в view (CREATE OR REPLACE VIEW не позволяет drop колонок),
-- но UI её игнорирует — амнистии в раскрытии больше не показываются вообще.
--
-- Причина отката: для bulk-amnesty все 10 budget_revoked_l2 пишутся в одной транзакции
-- с одинаковым timestamp. «Один виновник» не определяется. Лучше показать пустое раскрытие,
-- чем подсовывать 10 ложных или одну случайно выбранную.
-- Для обычного revoke (когда VPS-скрипт реально срывает серию из-за конкретной задачи)
-- виновник уже лежит в details.revoked_tasks — view возвращает его как раньше.
