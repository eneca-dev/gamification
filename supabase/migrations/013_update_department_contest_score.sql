-- Обновление VIEW: добавлена метрика contest_score = total_coins * (users_earning / total_employees)
-- Формула учитывает вовлечённость отдела: чем больше % сотрудников участвует, тем выше скор
...
SELECT
  ...
  ROUND(
    COALESCE(dc.total_coins, 0) * (COALESCE(dc.users_earning, 0)::numeric / dt.total_employees),
    1
  ) AS contest_score
FROM dept_totals dt
LEFT JOIN dept_coins dc ON dc.department_code = dt.department_code;

-- Обновление функции начисления бонуса: победитель определяется по contest_score
...
BEGIN
  ...
  -- Определяем победителя по формуле: total_coins * (active_users / total_employees)
  WITH dept_coins AS (
    SELECT
      wu.department_code,
      COUNT(DISTINCT t.user_id) AS users_earning,
