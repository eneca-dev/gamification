-- Master planner history view: revoke-события могут хранить список задач в двух местах:
--   - details.revoked_tasks       — нормальный путь (VPS-скрипт пишет сюда задачи, сорвавшие серию)
--   - details.original_details.tasks — fallback (bulk amnesty: тут лежит исходная серия из 10 задач)
-- COALESCE в поле revoked_tasks гарантирует, что UI получит непустой список и развернёт его.
...
SELECT
  ...
  COALESCE(
    ...
    el.details->'original_details'->'tasks'
  )                                               AS revoked_tasks,
  tr.coins,
